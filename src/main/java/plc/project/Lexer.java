...
public final class Lexer {
    ...
    public Token lexToken() {
        if (match("[A-Za-z_]")) return lexIdentifier();
        else if (peek("\\d")) return lexNumber();
        else if (peek("\\+", "\\d") || peek("\\-", "\\d")) return lexNumber();
        else if (peek("'")) return lexCharacter();
        else if (match("\"")) return lexString();
    ...
    public Token lexCharacter() {
        if (match( "'", ".", "'")) {
            ...
            throw new ParseException("Error: Exceeds Character Limit", chars.index);
        }
    }
